...
module.exports = class New_Library_Dialog {
  ...
  parseProtos(protos) {
    ...
    for (var i = 0; i < iMax; i++) {
      ...
      argPart = argPart.replace(/\s*REG\(\s*([a|d][0-7])\s*,/g, "REG($1;");  // tidy up argPart for easy parsing
      ...
      if (!(args[0].length == 0 || argPart == "void" || argPart == "VOID")) {
        ...
        for (var l = 0; l < lMax; l++) {
          ...
          var match = arg.match(/REG\(([a|d][0-7]);\s*(.*)\)/);
          if (match) {
            argReg = match[1];
            arg    = match[2];
          }
  ...
  assignRegs(fncSpecs) {
    var valueType = /^(?:(?:CONST|const)\s*)?(?:(?:unsigned\s+)?(?:char|short|long)|bool|double|float|U?(?:CHAR|BYTE|SHORT|COUNT|WORD|LONG)|BOOL|DOUBLE|FLOAT|(?:BYTE|WORD|LONG)BITS)$/;
  ...
  createInline(prjName, PRJNAME, baseName, fncSpecs, isPPC) {
    ...
    file += "// This file is a part of " + prjName + "\n\n#include <" + PPC.toLowerCase() + "inline/macros.h>\n\n";
    ...
    for (var i = 0; i < iMax; i++) {
      ...
      if (func.isVA) {
        ...
        if (tglVer.length == 0) continue; // if not found, no inline macro for this function
      ...
      if (func.isVA) {
        file += ") \\\n\t({_sfdc_vararg _args[] = {__VA_ARGS__}; " + tglVer + "(";
        for (var l = 0; l < lMax - 1; l++) {
          var arg = func.args[l];
          file += "(" + arg.name + "), "
        }
  ...
  createMakefile(prjName, prjname, PRJNAME, baseName, iFaceName, iFacePtr) {
    ...
    if (this.chkVecTable.checked) {
      ...
      var os3SplitPnt = /LFLAGS = -s -noixemul -nostdlib -nostartfiles/g; //also applies for MOS
      var os3SplitStr = "LFLAGS = -s -noixemul -nostdlib -nostartfiles";
      var os4SplitPnt = "LFLAGS = -noixemul -nostdlib -nostartfiles";

      var os3Objs = os3SplitStr + "\n  OBJS = " + prjname + ".o init.o";
