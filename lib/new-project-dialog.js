...
module.exports = class New_Project_Dialog {
  constructor() {
    ...
    this.element = document.createElement('div');
      ...
      // Project Description (visible on for MUI projects)
      this.descInsPnt = document.createElement('div');
        this.descDiv = document.createElement('div');
          ui.newLabel(this.descDiv, 'Description', 'h2');
          ui.newLabel(this.descDiv, 'Write a short description for your program:')
          this.descEditor = ui.newString(this.descDiv, this.disposables, null, 1, false);
        var gui = atom.config.get('language-amigaos-c.highlightPrefs.GUI');
        if (gui == "MUI" || gui == "MUI (with shortcut macros)")
          this.descInsPnt.appendChild(this.descDiv);
      this.element.appendChild(this.descInsPnt);
      ...
      this.slcType.addEventListener('change', () => {
        if (this.descInsPnt.childNodes) {
          if (this.slcType.selectedIndex == 2)
          this.descInsPnt.appendChild(this.descDiv);
          else if (this.descInsPnt.childNodes.length)
          this.descInsPnt.removeChild(this.descDiv);
        }
      });
  ...
  confirm() {
    ...
    if (this.nameEditor.element.hasFocus()) {
      if (this.slcType.selectedIndex == 2)
        this.descEditor.element.focus();
      else
        this.pathEditor.element.focus();
      return;
    }
    // if the user pressed enter on the description editor activate path editor
    if (this.descEditor.element.hasFocus()) {
      this.pathEditor.element.focus();
      return;
    }
    ...
    if (prjDir.length == 0) {
      this.showError("Please enter project directory!");
      this.pathEditor.element.focus();
      return;
    }
    ...
    if (this.slcType.value == 'Reaction') {
      ...
      var code = header + base[1] + this.createMUIStrings() + base[2] + mui_diffs[1] + base[3];
      if (this.chkSTD.checked) code += std;
    ...
    if (this.slcType.value != 'Reaction') code += make[7];
    code += make[8];
    ...
    atom.open({pathsToOpen: [prjFolder, mainFile], newWindow: true});
  ...
  createMUIStrings() {
    ...
    var cls = '\"\n';
    var auth = atom.config.get('language-amigaos-c.newPrjPrefs.authName');
    var contact = atom.config.get('language-amigaos-c.newPrjPrefs.authContact');
    var str = '#define AUTHOR          \"' + auth + cls;
    str += '#define COPYRIGHT       "\u00A9 ' + year + ' " AUTHOR\n';
    str += '#define CONTACT         \"' + contact + cls;
    str += '#define DESCRIPTION     \"' + this.descEditor.getText() + cls;
    return str;
  }
};
