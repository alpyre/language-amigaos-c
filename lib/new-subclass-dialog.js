...
module.exports = class New_SubClass_Dialog {
  ...
  confirm() {
    ...
    code += ca_name + base[6] + ca_name + base[7];
    ...
    if (this.chkUpdMake.checked) {
      ...
      // add dosupernew.o to OBJ list if it is not there
      console.log(code.indexOf('dosupernew.o'));
      if (code.indexOf('dosupernew.o') == -1) {
        code = code.replace(/OBJS = /g, 'OBJS = dosupernew.o ');
      }
    ...
    // copy dosupernew.c and its header into project if they do not exist
    if (!fs.existsSync(path.join(this.directory, 'dosupernew.c')))
      fs.copyFileSync(path.join(codesDir, 'dosupernew.c'), path.join(this.directory, 'dosupernew.c'));
    if (!fs.existsSync(path.join(this.directory, 'dosupernew.h')))
      fs.copyFileSync(path.join(codesDir, 'dosupernew.h'), path.join(this.directory, 'dosupernew.h'));
