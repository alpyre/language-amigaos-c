...
module.exports = {
    ...
    "autoCompPrefs": {
      ...
      "properties": {
        "autoComplete": {
          ...
          "description": "(Suggestions and snippets are currently available for OS3, OS4 (no structs), AHI and MUI functions.)",
          "type": "boolean",
          "default": true,
          "order": 1
        },
  ...
  createGrammar: function() {
    ...
    if (!(fs.existsSync(path.resolve(__dirname, '..', './grammars/amigaos-SDK-Symbols.cson')))) {
      ...
      // Write the new grammar files and activate them immediately
      fs.writeFile(path.resolve(__dirname, '..', './grammars/amigaos-SDK-FNC-Symbols.cson'), data2, (err) => {
        if (err) throw err;
        atom.grammars.loadGrammarSync(path.resolve(__dirname, '..', './grammars/amigaos-SDK-FNC-Symbols.cson'));
      });
      fs.writeFile(path.resolve(__dirname, '..', './grammars/amigaos-SDK-Symbols.cson'), data, (err) => {
        if (err) throw err;
        atom.grammars.loadGrammarSync(path.resolve(__dirname, '..', './grammars/amigaos-SDK-Symbols.cson'));
      });
      fs.writeFile(path.resolve(__dirname, '..', './completions/autoCompletions.JSON'), data3, (err) => {
        if (err) throw err;
        provider.loadCompletions();
      });
    }
